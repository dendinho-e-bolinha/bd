...
SELECT s1.name as subject1, t1.name as term1, s2.name as subject2, t2.name as term2
FROM Term t1, Term t2, Subject s1, Subject s2, TermGrades tg1, TermGrades tg2, GradeComponent gc1,
    GradeComponent gc2
WHERE tg1.grade=gc1.id AND tg1.subject=s1.id AND tg1.term=t1.id 
    AND tg2.grade=gc2.id AND tg2.subject=s2.id AND tg2.term=t2.id
AND gc1.id < gc2.id
    ...
    AND 1 in (
        WITH RECURSIVE Similar(id1, id2) AS (
            SELECT gc1.id,
                gc2.id
            WHERE (gc1.id, gc2.id) IN LayersSimilar
            UNION ALL
            SELECT c1.id,
                c2.id
            FROM Similar,
    ...
    ) ORDER BY t1.startDate DESC, t2.startDate DESC, s1.name ASC, s2.name ASC;
